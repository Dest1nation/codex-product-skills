...
6. 文件状态变化后，同步影响下游展示、接口输出、附件查看/下载功能权限和审计记录。
...
| S-005 | 敏感附件查看 | 授权用户 | 具备附件查看/下载功能权限 | 可查看或下载附件 |
...
| FR-006 | 权限控制 | P0 | 支持菜单权限、按钮/功能权限和列表数据权限控制 |
...
| --- | --- | --- | --- | --- |
| 一级菜单 | 文件管理 | 是 | 隐藏菜单或展示无权限页 | 菜单名称以最终信息架构为准 |
| 二级菜单 / 页面 | 文件生命周期管理 | 是 | 隐藏入口或展示无权限页 | 用于进入文件生命周期列表 |
| 二级菜单 / 页面 | 文件模板管理 | 是 | 隐藏入口或展示无权限页 | 用于维护文件模板 |

### 5.2 按钮权限

| 页面 | 按钮 / 功能点 | 是否需要权限控制 | 无权限表现 | 说明 |
| --- | --- | --- | --- | --- |
| 文件生命周期管理 | 新增 | 是 | 隐藏或不可点击 | 创建文件入口 |
| 文件生命周期管理 | 查看详情 | 是 | 隐藏或不可点击 | 详情入口通过按钮权限控制 |
| 文件生命周期管理 | 查看文件 | 是 | 隐藏或不可点击 | 敏感附件查看入口 |
| 文件生命周期管理 | 下载文件 | 是 | 隐藏或不可点击 | 敏感附件下载入口 |
| 文件生命周期管理 | 编辑 | 是 | 隐藏或不可点击 | 编辑入口；可编辑状态规则见第四章 |
| 文件生命周期管理 | 重新提交 | 是 | 隐藏或不可点击 | 驳回后重新提交入口；业务规则见第四章 |
| 文件生命周期管理 | 取消 | 是 | 隐藏或不可点击 | 取消入口；状态限制见状态机和第四章 |
| 文件生命周期管理 | 延期 | 是 | 隐藏或不可点击 | 延长结束日期入口 |
| 文件生命周期管理 | 提前归档 | 是 | 隐藏或不可点击 | 手动归档入口 |
| 文件生命周期管理 | 终止履约 | 是 | 隐藏或不可点击 | 手动失效入口 |
| 文件生命周期管理 | 续期/续签 | 是 | 隐藏或不可点击 | 创建续期文件入口 |
| 文件生命周期管理 | 导出 | 是 | 隐藏或不可点击 | 导出入口；导出范围见第四章 |
| 文件生命周期管理 | 导入 | 是 | 隐藏或不可点击 | 导入待处理或历史文件入口 |
...
| 文件模板列表 | 展示当前登录账号可管理范围内的模板 | 权限系统中的菜单权限和模板管理范围 | 按模板所属业务范围过滤 | 列表为空或展示无权限提示 |
